...
def make_promotion_table(common_type_rows: list[str]):
	"""Emit src/vatensor/gen/dtype_promotion_table.gen.hpp from the
	numpy.result_type rows produced by configure/generate_features.py."""
	# (a, b) → result, all in DType space.
	pairs: dict[tuple[DType, DType], DType] = {}
	for row in common_type_rows:
		a_code, b_code, r_code = row[0], row[1], row[2]
		pairs[(code_to_dtype[a_code], code_to_dtype[b_code])] = code_to_dtype[r_code]

	# Build a 13×13 initializer in DType enum order. `Invalid` (DType::DTypeMax)
	# fills any missing combinations — none should remain for our 13 supported
	# dtypes, but treat the cell as a sentinel just in case.
	dtypes_in_order = sorted(DType, key=lambda d: d.value)
	rows_cpp = []
	for a in dtypes_in_order:
		cells = []
		for b in dtypes_in_order:
			result = pairs.get((a, b))
			cells.append(f"DType::{result.name}" if result else "DType::DTypeMax")
		rows_cpp.append("\t\t{ " + ", ".join(cells) + " }")

	hpp = (
		"// Auto-generated by site_scons/site_tools/features.py — do not edit.\n"
		"// Numpy-style result_type table for the 13 supported dtypes.\n"
		"#ifndef VATENSOR_DTYPE_PROMOTION_TABLE_GEN_HPP\n"
		"#define VATENSOR_DTYPE_PROMOTION_TABLE_GEN_HPP\n"
		"\n"
		"#include \"vatensor/dtype.hpp\"\n"
		"\n"
		"namespace va {\n"
		f"\tinline constexpr std::array<std::array<DType, DType::DTypeMax>, DType::DTypeMax> _common_type_table = {{{{\n"
		+ ",\n".join(rows_cpp) +
		"\n\t}};\n"
		"}\n"
		"\n"
	...
	gen_path = pathlib.Path("src/vatensor/gen/")
	gen_path.mkdir(exist_ok=True)
	(gen_path / "dtype_promotion_table.gen.hpp").write_text(hpp)


def generate(env, sources):
	with pathlib.Path("configure/vfuncs.json").open("r") as f:
		raw = json.load(f)
		features = Features.from_json_obj(raw)

	make_promotion_table(raw.get("common_type_table", []))
