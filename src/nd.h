...
private:
    // Spits out an error when someone tries to construct a nd object.
    nd() {
        ERR_PRINT("Error: the nd class is not meant to be constructed.");
    }

    ~nd() {
        ERR_PRINT("Error: the nd class is not meant to be destructed.");
    }
